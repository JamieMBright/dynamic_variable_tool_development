...
for var=1:length(OMI_vars)
    ...
    if OMI_raw_process(var)==1
        ...
        % save the confidence
        filename=GetFilename(store,OMI_vars{var},years(y),OMI_prefix,'confidence');
        save(filename,'OMI_confidence','-v7.3');
        clear OMI_confidence
        % save the times
        filename=GetFilename(store,OMI_vars{var},years(y),OMI_prefix,'times_datevec');
        save(filename,'time_dayvecs','-v7.3');
        % save the one time latitudes and longitudes
        filename=GetFilename(store,OMI_vars{var},years(y),OMI_prefix,'latitudes');
